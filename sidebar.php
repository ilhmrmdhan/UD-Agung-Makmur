<aside id="sidebar" class="sidebar-transition w-64 bg-[#001f3f] text-white flex flex-col transition-all duration-300 overflow-hidden">
  <div id="sidebar-content"></div>
      ...
      <nav class="flex flex-col space-y-3 border border-gray-500 border-t-1 border-x-0 border-b-0 pt-4">
         <h2 class="px-2 py-2 text-gray-500">Utama</h2>
      ...
      Dashboard</a>
      <h2 class="px-2 py-2 text-gray-500">Tabel Master</h2>
        <a href="pegawai/pegawai.php" class="px-4 py-2 transition <?php echo basename($_SERVER['PHP_SELF']) == 'pegawai.php' ? 'bg-gray-100 text-black font-semibold' : ''; ?>"><i class="bi bi-person-standing"></i>
    Pegawai</a>
        <a href="customer/customer.php" class="px-4 py-2 transition <?php echo basename($_SERVER['PHP_SELF']) == 'customer.php' ? 'bg-gray-100 text-black font-semibold' : ''; ?>"><i class="bi bi-person-arms-up"></i>
    Pelanggan</a>
        <a href="item/item.php" class="px-4 py-2 transition <?php echo basename($_SERVER['PHP_SELF']) == 'item.php' ? 'bg-white text-black font-semibold' : ''; ?>"><i class="bi bi-box"></i></i>
    Barang</a>
        <a href="perusahaan/perusahaan.php" class="px-4 py-2 transition <?php echo basename($_SERVER['PHP_SELF']) == 'perusahaan.php' ? 'bg-white text-black font-semibold' : ''; ?>"><i class="bi bi-building"></i></i>
    Perusahaan</a>
        <a href="supplier/supplier.php" class="px-4 py-2 transition <?php echo basename($_SERVER['PHP_SELF']) == 'supplier.php' ? 'bg-white text-black font-semibold' : ''; ?>"><i class="bi bi-diagram-2-fill"></i>
    Pemasok</a> 
          <h2 class="px-2 py-2 text-gray-500">Tabel Pesanan</h2>
     <a href="pesanan/pesanan.php" class="px-4 py-2 transition <?php echo basename($_SERVER['PHP_SELF']) == 'Pesanan.php' ? 'bg-white text-black font-semibold' : ''; ?>"><i class="bi bi-card-list"></i>
    Nota Pembelian</a>
     <a href="nota/nota.php" class="px-4 py-2 transition <?php echo basename($_SERVER['PHP_SELF']) == 'nota.php' ? 'bg-white text-black font-semibold' : ''; ?>"><i class="bi bi-card-list"></i></i>
    Nota Penjualan</a>
      </nav>
    </div>
</aside>
