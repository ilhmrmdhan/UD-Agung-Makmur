...
<aside id="<?= $sidebarId ?>" class="fixed top-0 right-0 h-full w-96 mt-36 rounded-xl transform translate-x-full bg-[#001f3f] text-white border flex flex-col transition-transform duration-300 z-50 overflow-y-auto">
  <div class="px-4 pt-4 flex justify-between items-center">
    <h2 class="text-lg font-semibold">Ubah Customer</h2>
  ...
  <form action="" method="POST" class="space-y-4 pb-6">
    <div class="px-6">
      ...
      <input type="text" name="idcustomer" value="<?= $data['ID_CUSTOMER'] ?>" class="w-full text-black border px-4 py-2 mb-4 rounded" readonly>
    </div>

    <div class="px-6">
      <label class="block mb-1 font-medium">Nama Customer</label>
      <input type="text" name="nama" value="<?= $data['Nama_Customer'] ?>" class="w-full text-black border px-4 py-2 mb-4 rounded">
    </div>
...
if (isset($_POST['submit_ubah'])) {
  include '../koneksi.php';
  $nopol = $_POST['idcustomer'];
  $nama = $_POST['nama'];
  $tipe = $_POST['noHp'];

  $query = ("UPDATE customer SET Nama_Customer='$nama',No_Hp='$tipe' WHERE ID_CUSTOMER='$nopol'");
