...
<body class="flex h-screen bg-gray-200 font-sans">
  ...
  <?php include("perusahaantambah.php"); ?>

 <!-- Tombol Buka Sidebar -->
<main class="flex-1 p-6 m-20 mt-8 overflow-auto rounded-md bg-white">
  <div class="flex justify-between items-center mt-6">
    <h2 class="text-2xl font-bold mb-4">Table Perusahaan</h2>   
    ...
    <table id="example" class="w-full text-center border-collapse text-sm font-medium">
      <thead class="bg-blue-200">
        <tr>
          ...
          <th class="border px-4 py-2">Id Perusahaan</th>
          <th class="border px-4 py-2">Nama Perusahaan</th>
          <th class="border px-4 py-2">Alamat Perusahaan</th>
      ...
      <tbody>
       <?php
        ...
        while ($data = mysqli_fetch_array($query)) {
          $sidebarId = "sidebar-edit-" . $data['ID_PT'];
        ?>
          <tr class="hover:bg-gray-100">
            <td class="border px-4 py-2"><?php echo $no++; ?></td>
            <td class="border px-4 py-2"><?php echo $data['ID_PT']; ?></td>
            <td class="border px-4 py-2 text-left"><?php echo $data['Nama_PT']; ?></td>
            <td class="border px-4 py-2"><?php echo $data['Alamat_PT']; ?></td>
         ...
         <?php include("perusahaanhapus.php");  ?>
         <?php include("perusahaanubah.php");  ?>
         <?php } ?>
