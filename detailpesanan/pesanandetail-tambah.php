...
<body class="flex h-screen bg-gray-200 font-sans">
  ...
  <main class="flex-1 p-6 mx-4 mt-8 overflow-auto bg-white rounded-md shadow-inner">
    <div class="max-w-6xl mx-auto p-6 bg-white shadow-md rounded-lg">
      <h2 class="text-center text-xl font-bold mb-4">FORM DETAIL NOTA : <?php echo htmlspecialchars($data['NO_TRANSAKSI']);?></h2>
      <hr class="mb-6">
      ...
      <form action="" method="post">
        ...
        <div class="space-y-4">
          <div class="flex items-center">
            ...
            <input name="nota_no" class="flex-1 px-3 py-2 border rounded bg-gray-100" value="<?php echo htmlspecialchars($data['NO_TRANSAKSI']);?>" readonly>
          </div>
          <div class="flex items-center">
            <label class="w-32 font-medium">TANGGAL</label>
            <input type="date" name="tanggal" class="flex-1 px-3 py-2 border rounded bg-gray-100" value="<?php echo htmlspecialchars($data['Tanggal']);?>" readonly>
          </div>
          <div class="flex items-center">
            <label class="w-32 font-medium">KM MOBIL</label>
            <input type="number" name="km_mobil" class="flex-1 px-3 py-2 border rounded bg-gray-100" value="<?php echo htmlspecialchars($data['Total_Pembelian']);?>" readonly>
          </div>
          <div class="flex items-center">
            <label class="w-32 font-medium">NOPOL</label>
            <input type="text" name="nopol" class="flex-1 px-3 py-2 border rounded bg-gray-100" value="<?php echo htmlspecialchars($data['Nama_Supplier']);?>" readonly>
          </div>
          <div class="flex items-center">
            <label class="w-32 font-medium">KASIR</label>
            <input type="text" name="kasir" class="flex-1 px-3 py-2 border rounded bg-gray-100" value="<?php echo htmlspecialchars($data['Nama_PT']);?>" readonly>
          </div>
        </div>
        ...
        <div class="flex justify-end gap-4 mt-6">
          ...
          <a href="notadetail.php?NO_TRANSAKSI=<?php echo htmlspecialchars($data['NO_TRANSAKSI']); ?>" class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded">Kembali</a>
        </div>
      </form>
    </div>
  </main>
...
if (isset($_POST['proses'])) {
   ...
   $jumlah = (int)$_POST['jumlah']; 
    ...
    $total_sebelumnya = (int)$row_total['Total_Pembelian'];

    $query_insert = "INSERT INTO detail_pesanan VALUES('$nota_no','$no_barang','$banyaknya','$jumlah')";
