...
<aside id="<?= $sidebarId ?>" class="fixed top-0 right-0 h-full w-96 mt-36 rounded-xl transform translate-x-full bg-[#001f3f] text-white border flex flex-col transition-transform duration-300 z-50 overflow-y-auto">
  <div class="px-4 pt-4 flex justify-between items-center">
    <h2 class="text-lg font-semibold">Ubah Item</h2>
  ...
  <form action="" method="POST" class="space-y-4 pb-6">
    <div class="px-6">
      <label class="block mb-1 font-medium">No Barang</label>
      <input type="text" name="NO_ITEM" value="<?= $data['NO_ITEM'] ?>" class="w-full text-black border px-4 py-2 mb-4 rounded" readonly>
    </div>

    <div class="px-6">
      <label class="block mb-1 font-medium">Nama Barang</label>
      <input type="text" name="Nama_Barang" value="<?= $data['Nama_Barang'] ?>" class="w-full text-black border px-4 py-2 mb-4 rounded">
    </div>

    <div class="px-6">
      <label class="block mb-1 font-medium">Satuan</label>
      <input type="text" name="Satuan" value="<?= $data['Satuan'] ?>" class="w-full text-black border px-4 py-2 mb-4 rounded">
    </div>

    <div class="px-6">
      <label class="block mb-1 font-medium">Harga Satuan</label>
      <input type="number" name="Harga_Satuan" value="<?= $data['Harga_Satuan'] ?>" class="w-full text-black border px-4 py-2 mb-4 rounded">
    </div>
...
if (isset($_POST['submit_ubah'])) {
  ...
  $T_harga2 = $_POST['Satuan'];
  $T_harga2 = $_POST['Harga_Satuan'];

  $query = ("UPDATE item SET Nama_Barang='$nama', Satuan='$T_harga2',Harga_Satuan='$T_harga2' WHERE NO_ITEM='$nopol'");
